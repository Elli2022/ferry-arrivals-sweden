...
const parseArrivalsFromMarkdown = (
  ...
  for (const row of rows) {
    ...
    if (section === "in_port") {
      ...
      if (!vesselName || (!includeAllVessels && !isPassengerFerry(portId, vesselName))) {
        continue;
      }
      arrivals.push({
...
export default function App() {
  ...
  const [includeAllVessels, setIncludeAllVessels] = useState(false);
  ...
  const cacheKey = useMemo(
    () => `${selectedPort}-${dateIsoLocal(selectedDate)}-${includeAllVessels ? "all" : "passenger"}`,
    [selectedPort, selectedDate, includeAllVessels]
  );
    ...
    async (refreshMode = false) => {
      ...
      try {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
          ...
          const serverAggregated = await fetchServerAggregated(
            selectedPort,
            selectedDate,
            includeAllVessels
          );
          ...
          const parsedFromMainPage = parseArrivalsFromMarkdown(
            markdown,
            selectedPort,
            targetDate,
            includeAllVessels
          ...
          if (estUrl) {
            void (async () => {
              ...
              const estimateRows = parseEstimatePageMarkdown(
                estPack.text,
                selectedPort,
                targetDate,
                includeAllVessels
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.filterRow}>
        <Pressable
          style={[styles.filterChip, !includeAllVessels && styles.filterChipActive]}
          onPress={() => setIncludeAllVessels(false)}
        >
          <Text style={[styles.filterChipText, !includeAllVessels && styles.filterChipTextActive]}>
            Endast passagerarfärjor
          </Text>
        </Pressable>
        <Pressable
          style={[styles.filterChip, includeAllVessels && styles.filterChipActive]}
          onPress={() => setIncludeAllVessels(true)}
        >
          <Text style={[styles.filterChipText, includeAllVessels && styles.filterChipTextActive]}>
            Alla fartyg
          </Text>
